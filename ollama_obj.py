...
class ollamas:
    ...
    def init_chain(self, memory_length=10, read_documents=5):
        if self.FLAG_VDB_IS_EMPTY:
            ...
            self.memory = ConversationBufferWindowMemory(k=memory_length, return_messages=True)
            
            self.chain = RunnableMap({
                "history":  lambda x: self.memory.chat_memory.messages,
                "charText": lambda x: x["charText"],
                "talk": lambda x: x["talk"]
            }) | self.promt | self.LLM | StrOutputParser()
        else:
            self.promt = ChatPromptTemplate.from_messages([
                MessagesPlaceholder("history"),
                ("system", "This is from documents about you. \n\n {context} \n\n This is your role: {charText}"),
                ("user", "{talk}"),
            ])
            self.memory = ConversationBufferWindowMemory(k=memory_length, return_messages=True)
            self.VDBResponser = self.VDB.as_retriever(search_kwargs={"k": read_documents}) #get top 2 documents
            self.chain = RunnableMap({
